...
function create_product_node(product) {
    ...
    cat_description.textContent = product.description
    root.appendChild(cat_description)

    let cat_img = document.createElement('td')
    let cat_imgage = document.createElement('img')
    cat_imgage.setAttribute('class', 'product-imgage')
    cat_img.setAttribute('class', 'product-img')
    cat_imgage.src = product.img
    cat_img.appendChild(cat_imgage)
...
function doCreate() {
    ...
    create_products( name,description,img,price,cat_id)
}

function create_products(name,description,img,price,cat_id) {
    let params = { 'name': name,'description':description,'img':img, 'price':price,'category_id':cat_id }
...
function edit_products(id){
    $.getJSON(BASE_URL + '/product/show.php?id=' + id, function (res) {  
        ...
        let dom_description = document.getElementById("description")
        dom_name.value = product.name
...
function doUpdate(){
    ...
    let price = document.getElementById("price").value
    let img = document.getElementById("img").value
    update_products(id, name, description,img,price)
}

function update_products(id, name,description,img,price){
    let params = {'id': id,
    'name': name,
